...
def handler(job):
    ...
    for idx_counter, block in enumerate(timeline_data['timeline']):
        ...
        try:
            subprocess.run(f"ffmpeg -y -loop 1 -i {avatar_img_path} -i {output_wav} -vf 'scale=trunc(iw/2)*2:trunc(ih/2)*2' -c:v libx264 -pix_fmt yuv420p -c:a aac -b:a 320k -ac 2 -shortest {teacher_raw_mp4}", shell=True, check=True, capture_output=True, text=True)
